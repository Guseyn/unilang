...
const outDir = path.join(
  projectRoot,
  'examples/browser/web-app/static/js/unilang-in-worker-environment'
)
 ...
 * From: { "#unilang": "/js/unilang-in-worker-environment" }
 * To: { "#unilang/": "/js/unilang-in-worker-environment/" }
 */
...
try {
  ...
  console.log(`[create-unilang-in-worker-environment] Successfully processed ${srcDir}`)
  console.log(`[create-unilang-in-worker-environment] Output: ${outDir}`)
} catch (error) {
  console.error('[create-unilang-in-worker-environment] Error:', error.message)
  process.exit(1)
}
