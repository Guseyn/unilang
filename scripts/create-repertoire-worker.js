...
const outDir = path.join(
  projectRoot,
  'examples/browser/web-app/static/js/e-repertoire/repertoire-worker'
)
 ...
 * From: { "#repertoire": "/js/repertoire-worker" }
 * To: { "#repertoire/": "/js/repertoire-worker/" }
 */
...
function resolveSpecifier(specifier, map) {
  ...
  // Prefix match (e.g., '#repertoire/' matches '#repertoire/api.js')
...
try {
  ...
  console.log(`[create-repertoire-worker] Successfully processed ${srcDir}`)
  console.log(`[create-repertoire-worker] Output: ${outDir}`)
} catch (error) {
  console.error('[create-repertoire-worker] Error:', error.message)
  process.exit(1)
}
